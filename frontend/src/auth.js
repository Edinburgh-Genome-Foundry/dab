...
const LOGIN_URL = 'https://emmadb.genomefoundry.org/ICE-REST/rest/session/EMMA'
const REFRESH_TOKEN_URL = 'https://emmadb.genomefoundry.org/ICE-REST/rest/session/EMMA'
/**
...
const THIS_DOMAIN = window.location.hostname.split('.').slice(1).join('.') || 'localhost'
...
export default {
  ...
  login (creds, redirect) {
    ...
    console.log('LOGGING', creds)
    return Vue.http.post(LOGIN_URL, params, {
      headers: {
        Authorization: 'Basic ' + btoa(username + ':' + creds.password)
      },
      emulateJSON: true,
    }).then((response) => {
      this._storeToken(response)
      document.cookie = 'userId="' + username + '"; domain=' + THIS_DOMAIN
  ...
  logout () {
    ...
    document.cookie = 'sessionId=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=' + THIS_DOMAIN
    document.cookie = 'userId=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=' + THIS_DOMAIN
    this.login({
      username: store.state.settings.ANONYMOUS_USERNAME,
      password: store.state.settings.ANONYMOUS_PASSWORD
    })
    router.push({
      path: 'home'
    })
  ...
  _storeToken (response) {
    ...
    document.cookie = 'sessionId="' + response.body.data.token + '"; domain=' + THIS_DOMAIN
