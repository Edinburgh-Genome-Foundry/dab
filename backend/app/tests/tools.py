...
test_json_path = os.path.join("app", "tests", "test_json")
...
def load_json(path):
    if isinstance(path, (list, tuple)):
        path = os.path.join(*path)
    full_path = os.path.join(test_json_path, path)
...
class AppTestCase(TestCase):
    ...
    def run_job(self, job_timeout=20, json_request=None, **params):
        ...
        if json_request is None:
            ...
            json_request = json.dumps(params_with_defaults)
